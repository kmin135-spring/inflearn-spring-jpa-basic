...
* `select m.team from Member m` 은 묵시적 조인이 발생하므로 실제 수행 쿼리를 예측하기가 어려움
  * `select o.member.team from Order o` 면 조인이 2번 발생
  * 묵시적 조인이 많을 수록 뭐가 실행될지 예측이 점점 힘들어짐
  * 묵시적 조인은 항상 내부조인으로 동작
* 실무에서 묵시적 조인은 쓰지 말고 명시적 조인을 쓰자
  * `select m from Member m join m.team t`

### fetch join 기본

* 실무에서 매우 중요
  * N+1 문제는 대부분 fetch join으로 해결 가능
* SQL 조인에 있는 건 아니고 JPQL 에서 조인할 때 성능 최적화를 위해 제공하는 기술
* 명시적이고 의도된 EAGER 로딩 같은 느낌
* JPQL의 DISTINCT 는 SQL의 그것과 동시에 엔티티 중복을 제거해주는 기능도 가짐
  * `select distinct t from Team2 t join fetch t.members m`

---

* VS 일반조인은 실행시 연관된 엔티티를 함께 조회하지 않음
* `select m from Member m join m.team t` 를 실행했을 때 실제 실행되는 쿼리는...
  * `select m.* from ...`
* `select m from Member m join fetch m.team t` 를 실행했을 때 실제 실행되는 쿼리는...
  * `select m.*, t.* from ...`
...
* 페치 조인 대상에는 별칭을 줄 수 없다.
  * 하이버네이트는 가능한데, 가급적 하지 말자
  * `select t from Team2 t join fetch t.members m where m.age > 10`
  * 위를 예로 들면 Team2의 members 를 age 조건으로 끊어서 가져오려고 하고 있다.
  * 하지만 JPA의 기본 객체 컨셉은 Team2를 가져올거면 연관된 모든 members를 함께 가져와야한다.
    * 객체 그래프를 타고 갔을 때 모든 데이터가 들어있어야한다.
  * 예를들어 team의 member가 전체 10건인데 조건으로 인해 3개만 가져왔는데 다음 쿼리에서는 10개 모두 가져온다면 영속성 컨텍스트 입장에서는 뭐가 맞는지 판단할 수가 없음.
    * 게다가 일부만 가져온 상태로 DB에 그대로 반영되버리는 등 예상치 못한 결과가 발생할 수 있음
    * 위 예처럼 member 일부만 필요하면 그 일부를 얻는 쿼리를 별도로 날리는게 JPA 컨셉에 맞음.
  * 이상의 이유로 fetch join에는 별칭을 기본적으로 주면 안 됨.
    * 가끔 여러번 fetch join 해야할때 등의 상황에서 제한적으로 할 때도 있다고함
* 둘 이상의 컬렉션은 페치 조인 안 된다.
  * 어찌 가능한 경우도 있는데 이것도 정합성이 깨질 우려가 있다.
* 페이징 이슈
  * 1:1, N:1 은 페치 조인해도 건수가 동일하니 페이징 가능
  * 1:N은 N으로 뻥튀기되니 페이징 하면 절대 안 됨
    * 하이버네이트에서는 경고 로그를 남기고 페이징을 해주긴 하는데 동작 방식이 limit나 이런거 안 쓰고 전체 결과를 메모리에 올린다음 자체적으로 페이징하는 방식임
    * 장애 나기 딱 좋음. team이 100만개면...
    * 대안은 
      * N:1로 뒤집어서 쿼리한다거나 
      * `@BatchSize` 로 모아서 LAZY 로딩
      * 전용 DTO짜서 직접 쿼리를 짜는 건데 이것도 정제 작업이 쉽지 않음

---

* `@BatchSize` 란?
  * N+1 문제의 개선책
  * 예를 들어 BatchSize를 10으로 해두고 team 10개를 호출한뒤 members를 lazy로 순회하게 되면 member에 n번 쿼리가 나가는게 아니라 BatchSize만큼 몰아서 쿼리를 날려줌
  * LAZY 로딩하는 연관관계마다 애노테이션으로 달아둘 수도 있으나 글로벌 설정으로 1000 이하 정도로 잡는걸 권장한다고 함.

---

* 실무에서 글로벌 로딩 전략은 기본이 지연 로딩
* 최적화가 필요한 곳은 페치 조인 적용
  * 페치 조인은 글로벌 로딩 전략보다 우선됨
* JPA의 80% 가량의 성능 이슈는 N+1에서 발생하는데 거의 fetch join으로 해결된다.
* 페치 조인은 객체 그래프를 유지할 때 효과적
* 하지만 여러 테이블 조인이 필요한 통계화면처럼 엔티티가 가진 모양이 아닌 전혀 다른 결과를 내야할 경우 페치 조인 대신 일반 조인을 사용하고 알맞은 DTO로 반환하는 것이 적절


### Named 쿼리

* 미리 정의해놓고 사용하는 정적 쿼리
  * 애노테이션과 XML로 모두 정의 가능
* 애플리케이션 로딩 시점에 초기화 후 재사용 (캐싱)
* 애플리케이션 로딩 시점에 쿼리를 검증해줘서 오류가 있으면 앱 구동으로 바로 파악 가능
* 참고로 spring-data-jpa의 `@Query` 가 내부적으로 `@NamedQuery` 를 사용하는 것임
  * `@NamedQuery` 은 엔티티 클래스에 달아야 하는 반면에
  * `@Query`는 메서드 단위로 달 수 있기 때문에 실무에서는 이게 더 적절함. 

### Bulk 연산

* 변경건수가 적으면 변경감지를 쓰겠는데...
* 변경된 100건을 반영하려면?
  * 변경감지를 사용하면 100번 UPDATE가 필요
* 직접 insert, update, delete JPQL 작성하여 반영
  * 직접 DML 같은 JPQL을 날리면 바로 flush가 나간다.
* **주의사항** 벌크 연산은 영속성 컨텍스트를 무시하고 DB에 직접 쿼리하므로 정합성을 맞추기 위해 아래와 같이 해주자
  * 벌크 연산을 가장 먼저 실행하거나 (영속성 컨텍스트에 뭐가 들어오기 전에)
  * 벌크 연산 후 영속성 컨텍스트 초기화 후 사용
* spring-data-jpa의 `@Modifying` 이 JQPL의 벌크 연산을 래핑해둔건데 얘도 기본은 영속성 컨텍스트 초기화를 안 한다.
  * 대신 `clearAutomatically` 값을 `true`로 하면 초기화도 같이 해준다.